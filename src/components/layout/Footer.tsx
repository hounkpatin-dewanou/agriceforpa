  ...
  <div className="relative w-16 h-16 md:w-20 md:h-20 transition-transform duration-300 group-hover:scale-105">
    ...
    />
            ...
            <div className="flex gap-4">
              ...
              <Link href="#" className="bg-white/10 p-3 rounded-full hover:bg-white hover:text-agri-green transition-all">
                <Instagram size={20} />
              </Link>
          ...
          <div>
            ...
            <ul className="space-y-4">
              {['Accueil', 'À propos', 'Produits', 'Services', 'Contact'].map((item) => (
                <li key={item}>
                  <Link href={`/${item.toLowerCase().replace(' ', '-')}`} className="hover:translate-x-2 inline-block transition-transform opacity-90 hover:opacity-100">
                    {item}
                  </Link>
                </li>
              ))}
